...
- Assignees: Avenir Heavy (bold), 16px, text-muted for active / text-subtle for inactive
- Schedule line: Avenir Medium, 16px, same color as assignees
...
```tsx
<span className={`font-serif font-normal text-xl ${
  props.is_active ? "text-text-dark" : "text-text-subtle"
}`}>
  {props.name}
</span>
```
...
```tsx
<span className={`text-m font-avenir font-semibold ${
...
```tsx
<div className={`flex flex-row items-center text-m font-avenir ${
  props.is_active ? "text-text-muted" : "text-text-subtle"
}`}>
...
Key changes:
- Assignees: `text-sm` → `text-m`, added `font-semibold` (Figma: Avenir Heavy 16px)
- Schedule: `text-sm` → `text-m` (Figma: Avenir Medium 16px)
...
In the Figma design, all field labels ("Type", "Assignees", "Repeats", "Time", "Day(s) of the Week", "Message Template") use the serif font (Constantia) at 20px — not Avenir.
...
<div className="flex flex-col gap-3">
  ...
  <div className="flex flex-row flex-wrap gap-2">
    {["{firstName}", "{treeCount}", "{surveyLink}", "{treeNames}"].map((variable) => (
      <span
        key={variable}
        className="rounded-full bg-table-header px-4 py-1 font-mono text-m text-text-muted"
      >
        {variable}
      </span>
    ))}
...
export default function ReminderTextInput(props: ReminderLongTextInputProps) {
  return (
    <div className="flex flex-col w-full">
      ...
      {props.sublabel && <span className="font-avenir text-sm text-text-muted">{props.sublabel}</span>}
      <textarea
        className="w-full min-h-30 rounded-lg bg-white p-4 font-avenir text-m font-normal focus:outline-none mt-2 disabled:bg-table-header disabled:text-text-dark disabled:opacity-100"
...
- Label: added `font-semibold` (Figma: Avenir Heavy)
...
```tsx
className="flex flex-row items-center justify-between w-full h-10 rounded-full bg-white px-4 font-avenir text-m focus:outline-none hover:cursor-pointer disabled:cursor-default disabled:bg-table-header disabled:opacity-100"
```
...
```tsx
className="w-full h-10 rounded-2xl bg-white px-4 font-avenir text-m font-normal focus:outline-none disabled:bg-table-header disabled:text-text-dark disabled:opacity-100"
```
...
```tsx
className="w-full h-10 rounded-2xl bg-white px-4 pr-11 font-avenir text-m font-normal text-text-muted selection:bg-primary selection:text-text-light focus:outline-none disabled:bg-table-header disabled:text-text-dark disabled:opacity-100 [&::-webkit-calendar-picker-indicator]:hidden [&::-webkit-calendar-picker-indicator]:appearance-none"
```
